...
本项目系统性地应用了《数据结构与算法 B》课程中的以下知识：

| 课程知识点 | 本项目中的应用 |
|-----------|---------------|
| **图的基本概念** | 将城市路网建模为无向图，节点为路口/道路端点，边为道路 |
| **图的存储结构** | 使用 NetworkX 的邻接表存储，支持高效遍历和查询 |
| **图遍历（DFS/BFS）** | 提取连通分量、寻找叶子节点（剪枝） |
| **图简化（度2节点压缩）** | 将度数为 2 的非红绿灯节点合并，压缩冗余路网 |
| **图算法（环检测）** | 基于 `cycle_basis` 提取所有基础环 |
| **聚类（连通分量）** | 对短边图计算连通分量，实现空间上相近节点的合并 |
| **排序算法** | 对候选环按顺畅度、长度、距离等多维度排序 |
| **几何计算** | 计算节点角度、多边形面积/周长，评估环路圆度 |
| **交互式输入** | 用户指定起点、长度范围、搜索半径，动态筛选环路 |
| **可视化输出** | 使用 Folium 生成交互式地图，展示路网和检测到的环路 |
...
- 翻译结果自动归类到「论文翻译」文件夹
- 支持术语表导出
#### 注：翻译功能主要由pdf2zh实现，本项目仅增加了桌面便携传输功能
...
- **DeepSeek**：优化算法思路及生成python代码、github使用教学、撰写 README 文档
